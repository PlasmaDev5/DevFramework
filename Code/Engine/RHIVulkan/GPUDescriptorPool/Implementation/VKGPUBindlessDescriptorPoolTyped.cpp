...
plVKGPUBindlessDescriptorPoolTyped::plVKGPUBindlessDescriptorPoolTyped(plVKDevice& device, vk::DescriptorType type)
  : m_device(device)
  , m_type(type)
{
}

void plVKGPUBindlessDescriptorPoolTyped::ResizeHeap(plUInt32 req_size)
{
  if (req_size > max_bindless_heap_size)
  {
    PL_REPORT_FAILURE("Requested size for bindless pool more than max_bindless_heap_size");
    return;
  }
...
plVKGPUDescriptorPoolRange plVKGPUBindlessDescriptorPoolTyped::Allocate(plUInt32 count)
{
  auto index = m_empty_ranges.LowerBound(count);
  if (index != plInvalidIndex)
  {
    plUInt32 offset = m_empty_ranges[m_empty_ranges.GetKey(index)];
    plUInt32 size = m_empty_ranges.GetKey(index);

    // todo: update when pl can remove all entries with the given key
    while (m_empty_ranges.RemoveAndCopy(m_empty_ranges.GetKey(index)))
    {
    }

    return plVKGPUDescriptorPoolRange(*this, m_descriptor.set.get(), offset, size, m_type);
  }
  if (m_offset + count > m_size)
    ResizeHeap(plMath::Max(m_offset + count, 2 * (m_size + 1)));
  m_offset += count;
  return plVKGPUDescriptorPoolRange(*this, m_descriptor.set.get(), m_offset - count, count, m_type);
}

void plVKGPUBindlessDescriptorPoolTyped::OnRangeDestroy(plUInt32 offset, plUInt32 size)
{
  m_empty_ranges.Insert(size, offset);
}

vk::DescriptorSet plVKGPUBindlessDescriptorPoolTyped::GetDescriptorSet() const
{
